...
"""
@file
@brief A few functions about :epkg:`GPU`.
"""


def pyopencl_status():
    """
    Looks into :epkg:`GPU` and :epkg:`CPU` to
    see which card is available.
    Returns a string.
    ...
    .. runpython::
        ...
        from ensae_teaching_dl.faq.faq_gpu import pyopencl_status
    ...
    for plat in cl.get_platforms():
        ...
        for device in plat.get_devices():
            ...
            rows.append('Device - Max Clock Speed:  {0} Mhz'.format(
                catch(lambda: device.max_clock_frequency)))  # pylint: disable=W0640
            rows.append('Device - Compute Units:  {0}'.format(
                catch(lambda: device.max_compute_units)))  # pylint: disable=W0640
            rows.append('Device - Local Memory:  {0:.0f} KB'.format(
                catch(lambda: device.local_mem_size / 1024.0)))  # pylint: disable=W0640
            rows.append('Device - Constant Memory:  {0:.0f} KB'.format(
                catch(lambda: device.max_constant_buffer_size / 1024.0)))  # pylint: disable=W0640
            rows.append('Device - Global Memory: {0:.0f} GB'.format(
                catch(lambda: device.global_mem_size / 1073741824.0)))  # pylint: disable=W0640
            rows.append('Device - Max Buffer/Image Size: {0:.0f} MB'.format(
                catch(lambda: device.max_mem_alloc_size / 1048576.0)))  # pylint: disable=W0640
            rows.append('Device - Max Work Group Size: {0:.0f}'.format(
                catch(lambda: device.max_work_group_size)))  # pylint: disable=W0640
    rows.append('=' * 60)
    return "\n".join(rows)
